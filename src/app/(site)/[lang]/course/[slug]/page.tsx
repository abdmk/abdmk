...
export async function generateStaticParams() {
  const list = await getWorkshopsByKind('course');
...
export async function generateMetadata({
  ...
  const session = await getWorkshop(slug, 'course');
  if (!session) return {};
  ...
  return (
    <>
      ...
      <WorkshopDetail
        ...
        related={related}
        lang={lang}
        backHref={localePath(lang, '/courses')}
        backLabel={tr.workshops.coursesTitle}
      />
    </>
  );
}
